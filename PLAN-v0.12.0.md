...
| W1-6 | **Named-resource law completed across the struct** — `c.Data(name)` (mount-bound view, paths relative to the mount) and `c.Drive(name)` (handle-bound view: `Exists`/`Handle`/`Transport`) join Lock/Feature/Config/Action/Task/Command/Service/API/Entitled/RegistryOf. Every Registry-backed subsystem now speaks `c.X(name)`. Deliberately un-named (no registry axis): App, Fs, Process, Log, Error, IPC, Options, Cli. Parked as W3 decide items: `c.I18n(locale)` view, named Fs roots. | additive | this session |
